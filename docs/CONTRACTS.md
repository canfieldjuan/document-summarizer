...
post-response provenance check. Before generated bytes are parsed or accepted,
the adapter reads authenticated `/api/ps` and requires the running record for
the requested model name to identify exactly the digest admitted for that run.
