...
5. Reopening the database and running startup reconciliation resumes the saved phase without a duplicate remote job, lineage edge, child document, or child run.
...
7. Status and summary reads for the originally accepted run resolve to the admitted child, while history suppresses the internal failed root, so the existing UI monitors the real completed summary rather than rendering the handoff marker as a failure.
...
- `cargo test desktop::tests --lib` - 13 passed.
- `cargo test pipeline::workspace::tests --lib` - 10 passed.
