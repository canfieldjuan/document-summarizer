...
#[cfg(test)]
fn fixture_assignment_output(
    prompt: &SynthesisPrompt,
    claims: Vec<AttributedClaim>,
) -> Result<String, serde_json::Error> {
    let assignments = prompt
        .evidence
        .iter()
        .map(|item| {
            claims
                .iter()
                .position(|claim| claim.evidence_ids.contains(&item.evidence_id))
                .expect("Fixture must assign every evidence item")
        })
        .collect();
    serde_json::to_string(&RawClaimsResponse {
        claims: claims
            .into_iter()
            .map(|claim| RawCandidateClaim { text: claim.text })
            .collect(),
        assignments,
    })
}
...
#[cfg(test)]
fn fixture_pair_output(claims: Vec<AttributedCandidateClaim>) -> Result<String, serde_json::Error> {
    if claims
        .iter()
        .any(|c| c.candidate_ids.iter().any(|id| id == "candidate-foreign"))
    {
        // Deliberately invalid wire field: the production decoder forbids it.
        return serde_json::to_string(&AttributedCandidateClaimsResponse { claims });
    }
...
fn verification_prompt(
    ...
    verification_prompt_with_source_framing(claims, evidence, &HashMap::new())
}

fn verification_prompt_with_source_framing(
    claims: &[CitedClaim],
    evidence: &[EvidenceItem],
    source_framing_by_evidence: &HashMap<String, String>,
) -> Result<VerificationPrompt, PipelineFailure> {
    ...
    Ok(VerificationPrompt {
        claims: claims
            .iter()
            .map(|claim| {
                let source_framings = claim
                    .evidence_ids
                    .iter()
                    .filter_map(|evidence_id| {
                        source_framing_by_evidence
                            .get(evidence_id)
                            .map(String::as_str)
                    })
                    .collect::<HashSet<_>>();
                let framed_source_count = claim
                    .evidence_ids
                    .iter()
                    .filter(|evidence_id| source_framing_by_evidence.contains_key(*evidence_id))
                    .count();
                if source_framings.len() > 1
                    || (framed_source_count > 0 && framed_source_count != claim.evidence_ids.len())
                {
                    return Err(stage_failure(
                        PipelineStage::Verify,
                        "INVALID_SYNTHESIZED_DOCUMENT",
                        "A summary claim cannot mix source-framing labels during verification",
                        false,
                    ));
                }
                let source_framing = source_framings.into_iter().next().map(str::to_string);
                let evidence = claim
                    .evidence_ids
                    .iter()
                    .map(|evidence_id| {
...
fn verification_request(
    ...
    ModelRequest {
        stage: PipelineStage::Verify,
        ordinal,
        system_prompt: batch.system_prompt.clone(),
...
fn plan_verification_batches(
    ...
    for (prompt_claim, claim) in prompt.claims.iter().zip(claims) {
        ...
        let proposed_characters = verification_system_prompt(&proposed_prompt_claims)
            .chars()
            .count()
...
fn materialize_verification_batch(
    ...
    let system_prompt = verification_system_prompt(&prompt_claims);
    ...
    let model_facing_characters = system_prompt
        .chars()
        .count()
    ...
    match name.as_str() {
        ...
        SYNTHESIS_SCHEMA_NAME => {
            ...
            let claims = fixture_claim_groups(prompt.evidence.len(), prompt.minimum_claims)
                ...
                .collect();
            fixture_assignment_output(&prompt, claims)
        ...
        HIERARCHICAL_SYNTHESIS_SCHEMA_NAME => {
            ...
            let claims = fixture_claim_groups(prompt.candidates.len(), prompt.minimum_claims)
                .into_iter()
                .map(|indices| AttributedCandidateClaim {
...
mod tests {
        ...
        fn generate(&self, request: &ModelRequest) -> Result<ModelResponse, ModelRuntimeFailure> {
            ...
            let text = match &request.output_format {
                    ...
                    if name == HIERARCHICAL_SYNTHESIS_SCHEMA_NAME =>
                {
                    ...
                    fixture_pair_output(claims).expect("candidate claims should serialize")
                }
        ...
        for request in &requests {
            ...
            match &request.output_format {
                ModelOutputFormat::JsonSchema { name, schema } if name == SYNTHESIS_SCHEMA_NAME => {
                    ...
                    assert_eq!(
                        schema["anyOf"][0]["properties"]["claims"]["minItems"].as_u64(),
                        Some(prompt.minimum_claims as u64)
                    );
                    assert_eq!(
                        schema["anyOf"].as_array().unwrap().last().unwrap()["properties"]["claims"]
                            ["maxItems"]
                            .as_u64(),
                        Some(prompt.maximum_claims.min(prompt.evidence.len()) as u64)
                    );
                }
    ...
    #[test]
    fn structural_synthesis_and_reduction_preserve_durable_binding_and_rejections() {
        ...
        let calls = runtime.captured_requests().len();
        for bad in [
            candidates[..1].to_vec(),
            ...
            vec![
                candidates[0].clone(),
                SynthesisCandidate {
                    evidence_ids: vec!["other-batch-evidence".into()],
                    ..candidates[1].clone()
                },
            ],
            vec![
                candidates[0].clone(),
                SynthesisCandidate {
                    evidence_ids: vec![durable[2].clone(), durable[2].clone()],
                    ..candidates[1].clone()
                },
            ],
            vec![
                candidates[0].clone(),
                SynthesisCandidate {
                    evidence_ids: durable[2..].iter().rev().cloned().collect(),
                    ..candidates[1].clone()
                },
            ],
        ] {
            assert!(request_candidate_claims(
                &runtime,
                &analyzed,
                &bad,
                bounds,
            ...
            .is_err());
            assert_eq!(runtime.captured_requests().len(), calls);
        }
        for bounds in [
            ClaimBounds {
                minimum: 0,
        ...
        for request in runtime.captured_requests() {
            ...
            if name == SYNTHESIS_SCHEMA_NAME {
                assert_eq!(
                    schema["anyOf"][0]["properties"]["assignments"]["items"]["enum"],
                    json!([0])
                );
                assert_eq!(
                    schema["anyOf"][0]["properties"]["assignments"]["minItems"],
                    5
                );
            } else {
    ...
    #[test]
    fn verification_source_framing_is_explicit_isolated_and_mixed_evidence_fails_closed() {
        ...
        let evidence = ["framed", "ordinary"]
            .into_iter()
            .map(|name| EvidenceItem {
                evidence_id: format!("evidence-{name}"),
                chunk_id: "chunk-1".into(),
                block_id: format!("block-{name}"),
                claim_text: format!("{name} source."),
                exact_quote: format!("{name} source."),
                source_span: source_span.clone(),
            })
            .collect::<Vec<_>>();
        let framed_claim = CitedClaim {
            claim_id: "claim-framed".into(),
            text: "The document presents the following as a problem: Framed source.".into(),
            evidence_ids: vec![evidence[0].evidence_id.clone()],
        };
        let source_framing = HashMap::from([(evidence[0].evidence_id.clone(), "problem".into())]);
        let framed_prompt = verification_prompt_with_source_framing(
        ...
        .unwrap();
        assert_eq!(
            framed_prompt.claims[0].source_framing.as_deref(),
            Some("problem")
        );
        let (wire, _) = identifiers::verification_prompt(&framed_prompt.claims).unwrap();
        assert_eq!(
            serde_json::from_str::<Value>(&wire).unwrap()["claims"][0]["source_framing"],
            "problem"
        );
        assert!(verification_system_prompt(&framed_prompt.claims)
            .contains(SOURCE_FRAMING_VERIFICATION_INSTRUCTION));

        let ordinary_claim = CitedClaim {
            claim_id: "claim-ordinary".into(),
            text: "Ordinary source.".into(),
            evidence_ids: vec![evidence[1].evidence_id.clone()],
        };
        let ordinary_prompt = verification_prompt(
            std::slice::from_ref(&ordinary_claim),
        ...
        let error = verification_prompt_with_source_framing(
            std::slice::from_ref(&mixed_claim),
            &evidence,
            &source_framing,
        )
        .expect_err("framed and ordinary verification evidence must not mix");
        assert_eq!(error.code, "INVALID_SYNTHESIZED_DOCUMENT");
    }
    ...
    fn single_claim_capacity_preflights_deck_counts_before_generation() {
        ...
        assert_eq!(synthesis_request_user_character_limit(), Some(8_143));
    ...
    impl ModelRuntime for MaterialityRuntime {
        fn generate(&self, request: &ModelRequest) -> Result<ModelResponse, ModelRuntimeFailure> {
            ...
            if request.stage == PipelineStage::Synthesize && prior < self.synthesis_failures {
                response["assignments"].as_array_mut().unwrap().pop();
                if self.foreign_id {
    ...
    #[test]
    fn synthesis_rejects_missing_assignment_or_foreign_field_without_padding_or_retry() {
    ...
    fn synthesis_prompts_expose_the_application_claim_limit() {
        ...
        assert!(SYNTHESIS_SYSTEM_PROMPT.contains("one integer per supplied evidence item"));
        assert!(SYNTHESIS_SYSTEM_PROMPT.contains("every claim needs evidence"));
        assert!(HIERARCHICAL_SYNTHESIS_SYSTEM_PROMPT.contains("exactly two compatible candidates"));
        assert!(
            HIERARCHICAL_SYNTHESIS_SYSTEM_PROMPT.contains("exactly one claim consolidating BOTH")
        );
    }
